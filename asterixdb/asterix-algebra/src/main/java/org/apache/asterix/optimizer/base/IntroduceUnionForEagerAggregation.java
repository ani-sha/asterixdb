...
public class IntroduceUnionForEagerAggregation
    ...
    private boolean findAndApplyUnions(Mutable<ILogicalOperator> parentOpRef, IOptimizationContext context)
            throws AlgebricksException {
        ...
        for (int i = 0; i < op.getInputs().size(); i++) {
            ...
            if (inputOp.getAnnotations().containsKey("right_Side_of_Union"))
                continue;
            ...
            List<Pair<Mutable<ILogicalExpression>, Mutable<ILogicalExpression>>> filterExpressionList =
                    generateIntermediatePairs(valuePair);
    ...
    private boolean replaceAssignAndSelectOps(Mutable<ILogicalOperator> iteratingUnionOpRef,
            Pair<Mutable<ILogicalExpression>, Mutable<ILogicalExpression>> valuePair, IOptimizationContext context)
            throws AlgebricksException {
    ...
    private void createNewSelectOp(Mutable<ILogicalOperator> opRef,
            Pair<Mutable<ILogicalExpression>, Mutable<ILogicalExpression>> valuePair, IOptimizationContext context)
            throws AlgebricksException {
        SelectOperator oldSelectOp = (SelectOperator) opRef.getValue();
        ILogicalExpression leftValue =
                new ConstantExpression(new AsterixConstantValue(new AString(valuePair.getLeft().toString())));
        ILogicalExpression rightValue =
                new ConstantExpression(new AsterixConstantValue(new AString(valuePair.getRight().toString())));
        ...
        AbstractFunctionCallExpression newAndExpr =
    ...
    public static List<Pair<Mutable<ILogicalExpression>, Mutable<ILogicalExpression>>> generateIntermediatePairs(
            Pair<ConstantExpression, ConstantExpression> valuePair) throws AlgebricksException {
        ...
        if ((leftObject instanceof AString && rightObject instanceof AString)) {
            try {
                ...
                while (!startDate.isAfter(endDate.minusDays(1))) {
                    ILogicalExpression left =
                            new ConstantExpression(new AsterixConstantValue(new AString(startDate.toString())));
                    ILogicalExpression right = new ConstantExpression(
                            new AsterixConstantValue(new AString(startDate.plusDays(1).toString())));
    ...
    private Mutable<ILogicalOperator> createUnionOperator(Mutable<ILogicalOperator> opRef, IOptimizationContext context)
            throws AlgebricksException {
        ILogicalOperator op = opRef.getValue();
        ...
        List<Triple<LogicalVariable, LogicalVariable, LogicalVariable>> parentVarMap =
                ((UnionAllOperator) op).getVariableMappings();
    ...
    private void findLeftmostSelectOps(Mutable<ILogicalOperator> opRef, List<Mutable<ILogicalOperator>> selectOps,
            int maxSelects) {
