...
public class IntroduceSelectAccessMethodRule extends AbstractIntroduceAccessMethodRule {
    ...
    private boolean optimizeInteractiveArm(Mutable<ILogicalOperator> opRef, IOptimizationContext context)
            throws AlgebricksException {
    ...
    private boolean optimizeWRTGroupBy(Mutable<ILogicalOperator> opRef, IOptimizationContext context)
            throws AlgebricksException {
        ILogicalOperator op = opRef.getValue();
        if (op.getOperatorTag() == LogicalOperatorTag.EMPTYTUPLESOURCE) {
            return false;
    ...
    protected Index hasBTreeIndexForGroupByVariable(OptimizableOperatorSubTree subTree, LogicalVariable groupByVar,
        ...
        for (Index index : indexes) {
            if (index.getIndexType() == DatasetConfig.IndexType.BTREE) {
                ...
                if (fieldName.equals(keyFieldNames.get(0)))
                    return index;
    ...
    protected boolean checkAndApplyTheSelectTransformation(Mutable<ILogicalOperator> opRef,
        ...
        for (int i = 0; i < op.getInputs().size(); i++) {
            ...
            //                //change this line to switch side of interactive processing
