...
public class InteractiveSortRule implements IAlgebraicRewriteRule {
    ...
    public boolean rewritePre(Mutable<ILogicalOperator> opRef, IOptimizationContext context)
        ...
        if (operator.getOperatorTag() != LogicalOperatorTag.ORDER
                || operator.getOperatorTag() != LogicalOperatorTag.GROUP) {
            return false;
        }
        ...
        if (operator.getOperatorTag() == LogicalOperatorTag.ORDER)
            if (!hasBTreeIndexAccess(orderAttr, opRef, context))
                return false;
    ...
    private LogicalVariable GetIndexableAttribute(Mutable<ILogicalOperator> opRef, IOptimizationContext context)
            throws AlgebricksException {

        ILogicalOperator operator = opRef.getValue();
        if (operator.getOperatorTag() == LogicalOperatorTag.ORDER) {
            OrderOperator orderOperator = (OrderOperator) operator;
            List<Pair<OrderOperator.IOrder, Mutable<ILogicalExpression>>> orderKeys =
    ...
    public LogicalVariable findBestGByVar(Mutable<ILogicalOperator> opRef,
            List<Pair<LogicalVariable, Mutable<ILogicalExpression>>> gbyVarList, IOptimizationContext context)
            throws AlgebricksException {
    ...
    private Mutable<ILogicalOperator> createIndexBasedPlan(Mutable<ILogicalOperator> opRef,
    ...
    private Mutable<ILogicalOperator> cloneOperator(Mutable<ILogicalOperator> opRef, IOptimizationContext context)
            throws AlgebricksException {
    ...
    private List<LogicalVariable> extractJoinKeys(ILogicalExpression condition, IOptimizationContext context)
            throws AlgebricksException {
    ...
    private Mutable<ILogicalOperator> createIndexNestedLoopJoin(Mutable<ILogicalOperator> opRef,
            IOptimizationContext context) {
    ...
    private Mutable<ILogicalOperator> createIndexScanOperator(Mutable<ILogicalOperator> opRef,
            IOptimizationContext context) throws AlgebricksException {
        ...
        if (dataset.getDatasetType() == DatasetConfig.DatasetType.INTERNAL) {
            ...
            if (scanOperator.getOperatorTag() == LogicalOperatorTag.DATASOURCESCAN) {
                ...
                newBTreeParameters = new BTreeJobGenParams(chosenIndex.getIndexName(), DatasetConfig.IndexType.BTREE,
            ...
            recordType =
                    (ARecordType) metadataProvider.findTypeForDatasetWithoutType(recordType, metaRecordType, dataset);

            // Create the new operator to replace the DataSourceScan
            AbstractUnnestMapOperator chosenIndexUnnestOperator =
                    (AbstractUnnestMapOperator) AccessMethodUtils.createSecondaryIndexUnnestMap(dataset, recordType,
                            metaRecordType, chosenIndex, scanOperator.getInputs().get(0).getValue(), newBTreeParameters,
                            context, retainInput, false, false, ConstantExpression.MISSING.getValue());
    ...
    private Dataset findDataset(AbstractScanOperator scanOperator, BTreeJobGenParams originalBTreeParameters,
            IOptimizationContext context) throws AlgebricksException {
    ...
    public boolean hasBTreeIndexAccess(LogicalVariable var, Mutable<ILogicalOperator> opRef,
            IOptimizationContext context) throws AlgebricksException {
        ...
        while (!queue.isEmpty()) {
            ...
            if (currentOp.getOperatorTag() == LogicalOperatorTag.DATASOURCESCAN) {
                ...
                MetadataProvider mp = (MetadataProvider) context.getMetadataProvider();
                List<Index> indexes = mp.getDatasetIndexes(dataset.getDatabaseName(), dataset.getDataverseName(),
