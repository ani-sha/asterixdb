...
public class EstimatedCostComputationVisitor implements ILogicalOperatorVisitor<Pair<Double, Double>, Double> {
    ...
    @Override
    public Pair<Double, Double> visitIncrementalOrderOperator(IncrementalOrderOperator op, Double arg)
            throws AlgebricksException {
        double orderCost = 0.0;
        Pair<Double, Double> cardCost = op.getInputs().get(0).getValue().accept(this, arg);
    ...
    @Override
    public Pair<Double, Double> visitEagerMergeOperator(EagerMergeOperator eagerMergeOperator, Double arg)
            throws AlgebricksException {
        return annotate(this, eagerMergeOperator, arg);
    }
