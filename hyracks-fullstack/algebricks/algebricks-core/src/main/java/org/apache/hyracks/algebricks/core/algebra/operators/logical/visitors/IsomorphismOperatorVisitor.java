...
public class IsomorphismOperatorVisitor implements ILogicalOperatorVisitor<Boolean, ILogicalOperator> {
    ...
    @Override
    public Boolean visitIncrementalOrderOperator(IncrementalOrderOperator op, ILogicalOperator arg)
            throws AlgebricksException {
        AbstractLogicalOperator aop = (AbstractLogicalOperator) arg;
        if (aop.getOperatorTag() != LogicalOperatorTag.ORDER) {
            return Boolean.FALSE;
        }
        IncrementalOrderOperator orderOpArg = (IncrementalOrderOperator) copyAndSubstituteVar(op, arg);
        return op.getTopK() == orderOpArg.getTopK()
                && compareIOrderAndExpressions(op.getOrderExpressions(), orderOpArg.getOrderExpressions());
    }

    @Override
    public Boolean visitEagerMergeOperator(EagerMergeOperator thisOp, ILogicalOperator otherOp) {
        if (otherOp.getOperatorTag() != LogicalOperatorTag.EAGER_MERGE) {
            return false;
        }

        EagerMergeOperator thatOp = (EagerMergeOperator) otherOp;

        List<LogicalVariable> thisGroupBy = thisOp.getGroupByColumns();
        List<LogicalVariable> thatGroupBy = thatOp.getGroupByColumns();

        // Simple equality check on groupBy columns
        return thisGroupBy.equals(thatGroupBy);
    }
