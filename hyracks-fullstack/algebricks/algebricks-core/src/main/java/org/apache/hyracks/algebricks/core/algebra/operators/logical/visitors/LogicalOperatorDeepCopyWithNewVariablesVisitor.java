...
public class LogicalOperatorDeepCopyWithNewVariablesVisitor
    ...
    public ILogicalOperator visitSplitOperator(SplitOperator op, ILogicalOperator arg) throws AlgebricksException {
        Mutable<ILogicalExpression> branchingExpression =
                exprDeepCopyVisitor.deepCopyExpressionReference(op.getBranchingExpression());
    ...
    @Override
    public ILogicalOperator visitIncrementalOrderOperator(IncrementalOrderOperator op, ILogicalOperator arg)
            throws AlgebricksException {
        IncrementalOrderOperator opCopy =
                new IncrementalOrderOperator(deepCopyOrderExpressionReferencePairList(op.getOrderExpressions()),
                        deepCopyVariableList(op.getGroupByColumns()));
        deepCopyInputsAnnotationsAndExecutionMode(op, arg, opCopy);
        return opCopy;
    }

    @Override
    public ILogicalOperator visitEagerMergeOperator(EagerMergeOperator op, ILogicalOperator arg)
            throws AlgebricksException {
        // 1. Deep copy groupByColumns with new variables
        List<LogicalVariable> newGroupBy = new ArrayList<>();
        for (LogicalVariable oldVar : op.getGroupByColumns()) {
            LogicalVariable newVar = deepCopyVariable(oldVar);
            newGroupBy.add(newVar);
        }

        // 2. Recursively deep copy input
        ILogicalOperator inputCopy = op.getInputs().get(0).getValue().accept(this, arg);

        // 3. Create new EagerMergeOperator
        EagerMergeOperator newOp = new EagerMergeOperator(newGroupBy);
        newOp.getInputs().add(new MutableObject<>(inputCopy));
