...
public class OperatorDeepCopyVisitor implements ILogicalOperatorVisitor<ILogicalOperator, Void> {
    ...
    @Override
    public ILogicalOperator visitIncrementalOrderOperator(IncrementalOrderOperator op, Void arg)
            throws AlgebricksException {
        List<LogicalVariable> newVars = new ArrayList<>();
        return new IncrementalOrderOperator(this.deepCopyOrderAndExpression(op.getOrderExpressions()),
                this.deepCopyVars(newVars, op.getGroupByColumns()));
    }

    @Override
    public ILogicalOperator visitEagerMergeOperator(EagerMergeOperator op, Void arg) throws AlgebricksException {
        List<LogicalVariable> newVars = new ArrayList<>();
        return new EagerMergeOperator(this.deepCopyVars(newVars, op.getGroupByColumns()));
    }
}
