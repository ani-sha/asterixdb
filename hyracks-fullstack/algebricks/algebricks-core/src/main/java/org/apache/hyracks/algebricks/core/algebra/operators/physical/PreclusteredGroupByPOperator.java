...
public class PreclusteredGroupByPOperator extends AbstractPreclusteredGroupByPOperator {
    ...
    public void contributeRuntimeOperator(IHyracksJobBuilder builder, JobGenContext context, ILogicalOperator op,
        ...
        boolean isRewrittenMultiGroup = false;
        if (op.getAnnotations().containsKey("isRewrittenMultiGroup")) {
        ...
        if (op.getAnnotations().containsKey("isGlobal")) {
            if ((Boolean) op.getAnnotations().get("isGlobal") == true)
                isGlobal = true;
        }

        PreclusteredGroupOperatorDescriptor opDesc = new PreclusteredGroupOperatorDescriptor(spec, keys,
                comparatorFactories, aggregatorFactory, recordDescriptor, groupAll, framesLimit, isGlobal);
