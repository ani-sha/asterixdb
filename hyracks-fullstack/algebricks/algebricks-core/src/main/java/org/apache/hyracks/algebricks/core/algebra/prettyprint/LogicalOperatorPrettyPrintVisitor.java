...
public class LogicalOperatorPrettyPrintVisitor extends AbstractLogicalOperatorPrettyPrintVisitor<Integer>
    ...
    @Override
    public Void visitIncrementalOrderOperator(IncrementalOrderOperator op, Integer indent) throws AlgebricksException {
        addIndent(indent).append("order ");
        if (op.getTopK() != -1) {
            buffer.append("(topK: " + op.getTopK() + ") ");
        }
        ...
        pprintVarList(op.getGroupByColumns());
        return null;
    }

    @Override
    public Void visitEagerMergeOperator(EagerMergeOperator op, Integer indent) throws AlgebricksException {
        addIndent(indent).append("eager merge ");
        pprintVarList(op.getGroupByColumns());
        return null;
    }
