...
public class LogicalOperatorDotVisitor implements ILogicalOperatorVisitor<String, Boolean> {
    ...
    @Override
    public String visitIncrementalOrderOperator(IncrementalOrderOperator op, Boolean showDetails)
            throws AlgebricksException {
        stringBuilder.setLength(0);
        stringBuilder.append("order ");
        int topK = op.getTopK();
        ...
        printOrderExprList(op.getOrderExpressions());
        stringBuilder.append("groupVariables ");
        stringBuilder.append(op.getGroupByColumns());
        appendSchema(op, showDetails);
        appendAnnotations(op, showDetails);
        appendPhysicalOperatorInfo(op, showDetails);
        return stringBuilder.toString();
    ...
    @Override
    public String visitEagerMergeOperator(EagerMergeOperator op, Boolean showDetails) throws AlgebricksException {
        stringBuilder.setLength(0);
        stringBuilder.append("groupVariables ");
        stringBuilder.append(op.getGroupByColumns());
        appendSchema(op, showDetails);
