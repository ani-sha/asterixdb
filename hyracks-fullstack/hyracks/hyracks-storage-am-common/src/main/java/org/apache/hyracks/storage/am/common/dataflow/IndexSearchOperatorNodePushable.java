...
public abstract class IndexSearchOperatorNodePushable extends AbstractUnaryInputUnaryOutputOperatorNodePushable
    ...
    protected boolean isLeftMostTree;
    ...
    public IndexSearchOperatorNodePushable(IHyracksTaskContext ctx, RecordDescriptor inputRecDesc, int partition,
            ...
            boolean appendSearchCallbackProceedResult, byte[] searchCallbackProceedResultFalseValue,
            byte[] searchCallbackProceedResultTrueValue, ITupleProjectorFactory projectorFactory,
            ITuplePartitionerFactory tuplePartitionerFactory, int[][] partitionsMap, boolean isInteractive,
            boolean isLeftMostTree) throws HyracksDataException {
        this.ctx = ctx;
        this.isInteractive = isInteractive;
        this.isLeftMostTree = isLeftMostTree;
        this.appender = new FrameTupleAppender(new VSizeFrame(ctx), true);
        //TODO(partitioning) partitionsMap should not be null
        this.partitions = partitionsMap != null ? partitionsMap[partition] : new int[] { partition };
        for (int i = 0; i < partitions.length; i++) {
    ...
    protected void writeSearchResults(int tupleIndex, IIndexCursor cursor) throws Exception {
        ...
        while (cursor.hasNext()) {
            ...
            //            int keyStart = tuple.getFieldStart(keyFieldIndex) + 1;
            //            int keyLength = tuple.getFieldLength(keyFieldIndex) - 1;
            //            byte[] extractedKey = Arrays.copyOfRange(keyData, keyStart, keyStart + keyLength);
            //            byte[] extractedKey = extractFirstFieldBytes(tuple);
            ////            if (currentKey == null) {
            ////                currentKey = extractedKey;
            ////            }
            //
            //           // String key = deserializer.deserialize(keyData, keyStart, keyLength).toString();
            //            if(isInteractive && isLeftMostTree) {
            //
            ////                    if (!firstTuple && Arrays.compare(extractedKey, currentKey) != 0) {
            ////                        System.out.println("Emitting new sorted group: ");
            ////                        decodeIndexTuple(tuple); // Print current full tuple
            ////
            ////
            ////                        isEndOfKey = true;
            ...
            //                if (currentKey == null) {
            //                    currentKey = extractedKey;
            //                } else if (!Arrays.equals(currentKey, extractedKey)) {
            //                    // Group key changed
            //                    System.out.println(" Group key changed, flushing frame");
            //                    flushFrame();
            //                    currentKey = extractedKey;
            //                }
            //
    ...
    public void nextFrame(ByteBuffer buffer) throws HyracksDataException {
        if (!finished) {
            ...
            try {
                //                if (isInteractive &&  shouldStopProcessing("/Users/pratyoy/Downloads/HybridExecution/B2ISignal")) {
                //                    finished = true;
                //                    System.out.println("Blocking plan is ready to produce answers");// Mark as finished
                //
                //                    Throwable failure = null; // Track any failures during early exit
                //
                //                    try {
                //                        failure = releaseResources(failure); // Release resources
                //                        failure = CleanupUtils.close(writer, failure); // Close writer
                //                    } catch (Exception e) {
                //                        failure = e; // Capture failure
                //                    }
                //
                //                    if (failure != null) { // If any failure occurred, throw it
                //                        throw HyracksDataException.create(failure);
                //                    }
                //
                //                    return; //
            ...
            if (isInteractive) {
                //System.out.println("Sleeping to simulate slower interactive process");
                //Thread.sleep(60000); // Sleep for 30,000 milliseconds (30 seconds)
            }
        }
    }
            ...
            try {
                if (indexes[i] != null) {
                    try {
                        ...
                        if (!ise.getMessage().contains("state DESTROYED")) {
                            throw ise;
                        }
                        // else ignore the "DESTROYED" case
                    }
    ...
    private boolean shouldStopProcessing(String filePath) {
        try {
    ...
    private void decodeIndexTuple(ITupleReference tuple) {
        ...
        while (offset < end) {
            ...
            String fieldStr = "Field " + fieldNum + " (Tag=0x" + String.format("%02X", tag) + "): ";

            switch (tag) {
                case 0x0D: { // STRING (ATypeTag.STRING = 13)
                    int utfLen = ((data[offset] & 0xFF) << 8) | (data[offset + 1] & 0xFF);
                    offset += 2;
                    String str = new String(data, offset, utfLen, StandardCharsets.UTF_8);
                    fieldStr += "\"" + str + "\"";
                    offset += utfLen;
                    break;
                }
                case 0x03: { // INT32
                    int val = ((data[offset] & 0xFF) << 24) | ((data[offset + 1] & 0xFF) << 16)
                ...
                case 0x04: { // BIGINT
                    long val = 0;
                    for (int j = 0; j < 8; j++) {
                        val = (val << 8) | (data[offset + j] & 0xFF);
                    }
                    fieldStr += val;
                    offset += 8;
                    break;
                }
                case 0x11: { // DATE (ATypeTag.DATE = 17)
                    int days = ((data[offset] & 0xFF) << 24) | ((data[offset + 1] & 0xFF) << 16)
                            | ((data[offset + 2] & 0xFF) << 8) | (data[offset + 3] & 0xFF);
                    fieldStr += java.time.LocalDate.ofEpochDay(days);
                    offset += 4;
                    break;
                }
                default:
                    fieldStr += "UnknownTag";
                    // Skip for safety — you'll need length info if it's a known type
                    return;
            }
    ...
    private byte[] extractFirstFieldBytes(ITupleReference tuple) {
        byte[] data = tuple.getFieldData(0);
        ...
        switch (tag) {
            ...
            case 0x04: // BIGINT
                ...
                break;
            case 0x0D: // STRING
                int strLen = ((data[offset + 1] & 0xFF) << 8) | (data[offset + 2] & 0xFF);
                totalLen = 1 + 2 + strLen;
                break;
            default:
        ...
        return Arrays.copyOfRange(data, offset, offset + totalLen);
    }
